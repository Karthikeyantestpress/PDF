...
function loadSavedAnnotationData() {
  const SAVED_ANNOTATIONS = LocalStorageUtils.getObject('pdf-content-1');
  console.log('saved annotations');
  console.log(SAVED_ANNOTATIONS);
  ...
  if (SAVED_ANNOTATIONS) {
    setTimeout(() => {
      ...
      // Set all saved annotations
      pdfDocument.annotationStorage.setAll(SAVED_ANNOTATIONS);

      // Re-render the PDF to apply annotations
      reRenderPDFWithAnnotations(pdfDocument);
    }, 5000);
  }
}

function reRenderPDFWithAnnotations(pdfDocument) {
  ...
  for (let i = 1; i <= numPages; i++) {
    pdfDocument.getPage(i).then(page => {
      ...
      renderTask.promise.then(() => {
        console.log(`Page ${i} re-rendered with annotations`);
      });
    });
  }
}
...
function saveAnnotation() {
  ...
  console.log('data to annotate');
  console.log(ANNOTATIONS);
  LocalStorageUtils.setObject('pdf-content-1', ANNOTATIONS);
}

window.addEventListener('DOMContentLoaded', loadSavedAnnotationData);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  PDFViewerApplication.open({ url: file }).then(() => {
    // Listen for the document loaded event
    PDFViewerApplication.eventBus.on('documentloaded', loadSavedAnnotationData);
  });

  window.addEventListener('click', saveAnnotation);
});